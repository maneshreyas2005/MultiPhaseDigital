...
const OurServicesPage = () => {
    ...
    return (
        ...
        >
            {/* Heading */}
            <div
                className={`flex flex-col text-center ${sectionInView ? "animate-slide-up" : "opacity-100"}`}
                    ...
                    {services.map((service, index) => (
                        ...
                        >
                            <div className="space-y-3"
                                ...
                                {/* Number */}
                                <div
                                    className={`number font-light tracking-wider transition-all duration-400 overflow-hidden md:mb-5
                                        ${service.highlighted
                                        ...
                                        ${sectionInView ? "animate-slide-up" : "opacity-100"}
                                    `}
                                    style={{
                                        fontFamily: 'Constantine',
    ...
    mt-[-25px] md:mt-[-40px] lg:mt-[-42px] xl:mt-[-50px]   
    ${service.highlighted
                                            ? 'bg-gradient-to-r from-[#FF738B] via-[#9116E7] to-[#1B192E] bg-clip-text text-transparent'
  ...
  `}
                ...
                {/* Mobile View */}
                {/* <div
                    className="min-[500px]:hidden flex flex-col justify-center items-center w-full px-4 sm:px-0 ml-[1.5rem]"
                    style={{
                        gap: 'clamp(2rem, 4vw, 4rem) clamp(1.5rem, 3vw, 3rem)'
                    }}
                >
                    {services.map((service, index) => (
                        <div
                            key={index}
                            className="group cursor-pointer transition-all duration-300 ease-out flex flex-col items-center"

                        >
                            <div className="space-y-3 h-[80vh] flex flex-col justify-center items-center ml-[-30px]"
                                onMouseEnter={(e) => {
                                    const numberEl = e.currentTarget.querySelector('.number');
                                    if (numberEl) numberEl.style.WebkitTextStroke = "0px transparent";
                                }}
